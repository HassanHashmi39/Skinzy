...
router.get('/:id/slots', protect, async (req, res) => {
    try {
        ...
        if (doctor.availability && doctor.availability.length > 0) {
            ...
            // Safety fallback: if all days are inactive (e.g., doctor just registered but hasn't configured schedule),
            // use Mon-Sat defaults so patients can still book
            const hasAnyActiveDay = Object.values(availabilityMap).some(v => v === true);
            if (!hasAnyActiveDay) {
                console.log(`[SLOTS] All days inactive for ${doctor.name}, applying Mon-Sat fallback`);
        ...
        for (let i = 0; i < 14; i++) {
            // Fix DST boundary shift bug by adding days rather than absolute milliseconds
            ...
            if (availabilityMap[dayIdx]) {
                ...
                workingSlots.forEach(time => {
                    ...
                    // Robust time parsing protecting against crashes from malformed slots (e.g. support both 12-hour and 24-hour slots)
                    const timeMatch = time.trim().match(/^(\d{1,2}):(\d{2})(?:\s*(AM|PM))?$/i);
                    if (!timeMatch) return; // Skip invalid slot configurations

                    const [_, hourStr, minStr, period] = timeMatch;
