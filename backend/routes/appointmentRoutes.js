...
router.post('/', protect, patientOnly, async (req, res) => {
    try {
        ...
        // 🔴 Real-time: push notification instantly to the doctor via socket
        if (req.io) {
            req.io.to(doctorId.toString()).emit('new_notification', {
                _id: notif._id,
                type: 'appointment',
                title: notif.title,
...
router.patch('/:id/status', protect, async (req, res) => {
    try {
        ...
        // 🔴 Real-time: push status-change notification instantly via socket
        if (req.io) {
            req.io.to(notifyUser.toString()).emit('new_notification', {
                type: type,
                title: title,
                message: message,
                isRead: false,
...
router.post('/:id/messages', protect, async (req, res) => {
    try {
        ...
        if (appointment.patient.toString() !== req.user.id && appointment.doctor.toString() !== req.user.id) {
            return res.status(403).json({ message: 'Access denied: Unauthorized to post messages in this appointment' });
        }
...
// @access  Private (Doctor only)
router.get('/:id/patient-details', protect, doctorOnly, async (req, res) => {
    try {
