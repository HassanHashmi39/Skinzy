...
router.put('/:userId', protect, async (req, res) => {
    try {
        // Whitelist allowed fields to prevent mass assignment (e.g., self-escalation of role or verificationStatus)
        const allowedFields = [
            'name', 'phone', 'city', 'location', 'age', 'dateOfBirth', 'gender',
            'skinType', 'skinTone', 'mainConcerns', 'allergies', 'chronicConditions',
            'currentMedications', 'shareDataWithDoctors', 'profileImage', 'bio',
        ...
        const { profileImage, ...otherUpdates } = updates;
