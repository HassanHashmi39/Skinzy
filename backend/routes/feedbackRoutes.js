...
router.post('/', protect, async (req, res) => {
    try {
        ...
        // Concurrency-safe ratings update: dynamically aggregate ratings for this doctor
        const allFeedbacks = await Feedback.find({ doctor: appointment.doctor });
        const reviewsCount = allFeedbacks.length;
        const avgRating = allFeedbacks.reduce((sum, f) => sum + f.rating, 0) / reviewsCount;
