...
import { getConversation, getCurrentUser, getRecentChats, markMessagesAsRead, sendChatMessage, SOCKET_URL, getDoctors } from '../../../utils/api';
...
export function PatientChatScreen({ onBack, initialDoctorId }: { onBack: () => void; initialDoctorId?: string }) {
  const [conversations, setConversations] = useState<Conversation[]>([]);
  const [selectedChatUser, setSelectedChatUser] = useState<ChatUser | null>(null);
  const [messages, setMessages] = useState<Message[]>([]);
  const [messageText, setMessageText] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [allDoctors, setAllDoctors] = useState<ChatUser[]>([]);
  ...
  const fetchAllDoctors = async () => {
    try {
      ...
      if (data && data.doctors) {
        ...
        if (initialDoctorId) {
          const doc = data.doctors.find((d: any) => d._id === initialDoctorId);
          if (doc) {
            setSelectedChatUser(doc);
            loadConversation(doc._id);
          }
        }
      }
    } catch (e) {
      console.error('Failed to load all doctors:', e);
    }
  };
  ...
  const loadConversation = async (userId: string) => {
    try {
      ...
      await markMessagesAsRead(userId).catch(console.error);
      // Refresh chats without setloading to prevent stuttering
      const fresh = await getRecentChats();
  ...
  const selectedChatUserRef = useRef<ChatUser | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', async (msg: Message) => {
      ...
      if (currentSelected && (senderId === currentSelected._id || msg.receiver === currentSelected._id)) {
        setMessages(prev => {
          if (prev.some(m => m._id === msg._id)) return prev;
          const filtered = prev.filter(m => m._id.length > 15 || m.content !== msg.content);
          return [...filtered, msg];
        });

        if (senderId === currentSelected._id) {
          await markMessagesAsRead(currentSelected._id).catch(console.error);
        }
      }
  ...
  const existingChatUserIds = new Set(conversations.map(c => c.user._id));
  const filteredSearchDoctors = searchQuery.trim().length > 0
    ? allDoctors.filter(doc => 
        !existingChatUserIds.has(doc._id) && 
          ...
          {loadingMessages || !currentUser ? (
            ...
            messages.map((msg, index) => {
              ...
              const senderId = typeof msg.sender === 'object' && msg.sender !== null
                ? (msg.sender as { _id: string })._id
                : msg.sender as string;
              const isMine = currentUser && senderId ? String(senderId) === String(currentUser._id) : false;
              const senderName = isMine ? 'You' : (typeof msg.sender === 'object' && msg.sender !== null && 'name' in msg.sender ? (msg.sender as any).name : selectedChatUser.name);
  ...
  const renderSidebar = () => (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <ScrollView style={{ flex: 1 }}>
        {loadingChats ? (
          <ActivityIndicator size="large" color="#9333ea" style={{ marginTop: 40 }} />
        ) : filteredConversations.length === 0 && filteredSearchDoctors.length === 0 ? (
          ...
          <>
            ...
            {filteredSearchDoctors.length > 0 && (
              <>
                <View style={{ padding: 16, backgroundColor: '#f9fafb', borderBottomWidth: 1, borderColor: '#f3f4f6' }}>
                  <Text style={{ fontSize: 12, fontWeight: '700', color: '#7c3aed', textTransform: 'uppercase', letterSpacing: 1.5 }}>
                    Start New Chat
                  </Text>
                </View>
                {filteredSearchDoctors.map(doc => (
                  <TouchableOpacity key={doc._id}
                    onPress={() => { setSelectedChatUser(doc); loadConversation(doc._id); }}
                    style={{ flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderColor: '#f3f4f6', backgroundColor: '#fff', gap: 12 }}
                  >
                    <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: getAvatarColor(doc.name), alignItems: 'center', justifyContent: 'center' }}>
                      <Text style={{ color: '#fff', fontSize: 16, fontWeight: 'bold' }}>{getInitials(doc.name)}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontWeight: 'bold', color: '#111827' }}>{doc.name}</Text>
                      <Text style={{ color: '#6b7280', fontSize: 12, marginTop: 2 }}>{doc.specialization || 'Doctor'}</Text>
                    </View>
                    <View style={{ backgroundColor: '#e0e7ff', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 }}>
                      <Text style={{ color: '#4338ca', fontSize: 11, fontWeight: '700' }}>Start Chat</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </>
...
export default function ChatPage() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <PatientChatScreen onBack={() => router.push('/patient/dashboard')} />
    </SafeAreaView>
  );
}
