...
import { getRecentChats, getConversation, sendChatMessage, markMessagesAsRead, markAllMessageNotificationsAsRead, getCurrentUser, SOCKET_URL } from '../../utils/api';
...
export function DoctorChat({ onBack }: DoctorChatProps) {
  ...
  useEffect(() => { 
    fetchCurrentUser(); 
    fetchRecentChats();
    // Clear all message notification badges immediately on mount
    markAllMessageNotificationsAsRead().catch(e => console.error('Failed to clear message badges:', e));
  }, []);
  ...
  // Sync selected chat user ref
  useEffect(() => {
    selectedChatUserRef.current = selectedChatUser;
  }, [selectedChatUser]);

  // Setup Socket.io connection and message listener
  useEffect(() => {
