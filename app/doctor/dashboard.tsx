...
import { markAllMessageNotificationsAsRead } from '../../utils/api';
...
function DoctorDashboard() {
  ...
  // Real-time socket for badge count
  useEffect(() => {
    let mounted = true;
    const setupSocket = async () => {
      try {
        ...
        socket.on('new_notification', (notif: any) => {
          ...
          // Increment unread badge when new notification arrives
  ...
  const fetchData = async () => {
    try {
      ...
      if (profile && profile.user) {
        const role = profile.user.role || profile.user.userType;
        if (role && role !== 'doctor') {
          if (Platform.OS === 'web') {
            window.alert('Access Denied: Patients cannot access the doctor dashboard.');
          } else {
            Alert.alert('Access Denied', 'Patients cannot access the doctor dashboard.');
          }
      ...
      if (notifs && notifs.notifications) {
        const unread = notifs.notifications.filter((n: any) => !n.isRead && n.type !== 'message' && n.type !== 'new_message').length;
  ...
  if (showNotifications) return <DoctorNotifications 
    onBack={() => {
      setShowNotifications(false);
      setUnreadNotifications(0); // Clear badge when panel is closed
    }}
    onNewNotification={(count) => setUnreadNotifications(prev => prev + count)}
  />;
          ...
          <View className="flex-row items-center justify-between mb-6">
            ...
            <View className="flex-row gap-4 pt-2">
              <TouchableOpacity onPress={async () => { 
                setShowChat(true); 
                setUnreadMessages(0);
                try { await markAllMessageNotificationsAsRead(); } catch(e) {}
              }} className="items-center gap-1.5 group">
                <View className="p-3 bg-gray-50 rounded-xl relative shadow-sm border border-gray-100 transition-colors group-hover:bg-purple-100">
                ...
                <Text className="text-[10px] font-extrabold text-gray-500 uppercase tracking-wide group-hover:text-purple-600 transition-colors">Chat</Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={() => setShowNotifications(true)} className="items-center gap-1.5 group">
                <View className="p-3 bg-gray-50 rounded-xl relative shadow-sm border border-gray-100 transition-colors group-hover:bg-purple-100">
                ...
                <Text className="text-[10px] font-extrabold text-gray-500 uppercase tracking-wide group-hover:text-purple-600 transition-colors">Notifs</Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={() => setActiveTab('settings')} className="items-center gap-1.5 group">
                <View className="p-3 bg-gray-50 rounded-xl shadow-sm border border-gray-100 transition-colors group-hover:bg-purple-100">
                  <Settings color="#4B5563" size={20} />
                </View>
                <Text className="text-[10px] font-extrabold text-gray-500 uppercase tracking-wide group-hover:text-purple-600 transition-colors">Settings</Text>
              </TouchableOpacity>
            </View>
          </View>
