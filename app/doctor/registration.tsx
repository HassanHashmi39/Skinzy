...
function DoctorRegistration({ onNavigate, onRegistrationComplete }: DoctorRegistrationProps) {
  ...
  const [validationStatus, setValidationStatus] = useState<Record<string, {
      ...
      license: { status: 'idle' },
      cnicFront: { status: 'idle' },
      cnicBack: { status: 'idle' },
  ...
  const handleFileUpload = async (type: 'license' | 'cnicFront' | 'cnicBack' | 'certificate') => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        // Frontend validation: Check extension
        const ext = pickedAsset.name.split('.').pop()?.toLowerCase();
        const allowedExtensions = ['pdf', 'jpg', 'jpeg', 'png'];
        if (!ext || !allowedExtensions.includes(ext)) {
          setValidationStatus(prev => ({
            ...prev,
            [type]: { status: 'invalid', message: 'Allowed file formats: PDF, JPG, JPEG, PNG.' }
          }));
          return;
        }

        // Frontend validation: Check size (max 5MB)
        if (pickedAsset.size && pickedAsset.size > 5 * 1024 * 1024) {
          setValidationStatus(prev => ({
            ...prev,
            [type]: { status: 'invalid', message: 'File size must be under 5MB.' }
          }));
          return;
        }
        ...
        try {
          const res = await api.validateDocument(pickedAsset, type);
          if (res && res.valid) {
            setValidationStatus(prev => ({
              ...prev,
              [type]: { 
                status: 'valid', 
                message: type === 'license' ? '✅ Valid Medical Document' :
                         type === 'cnicFront' ? '✅ Valid CNIC Front' :
                         type === 'cnicBack' ? '✅ Valid CNIC Back' : '✅ Valid Document' 
              }
            }));
          } else {
            setValidationStatus(prev => ({
              ...prev,
              [type]: { status: 'invalid', message: res.message || 'Invalid Upload' }
            }));
          }
        } catch (validationErr: any) {
          console.error('Document validation failed:', validationErr);
          setValidationStatus(prev => ({
            ...prev,
            [type]: { status: 'invalid', message: validationErr.message || 'Invalid Upload' }
          }));
        }
      }
    } catch (err) {
  ...
  const handleNext = () => {
    ...
    if (step === 2) {
      ...
      } else if (validationStatus.license.status !== 'valid') {
        newErrors.license = validationStatus.license.message || 'Medical License must pass validation';
      }

      if (!documents.cnicFront) {
        newErrors.cnicFront = 'CNIC Front Side is required';
      } else if (validationStatus.cnicFront.status !== 'valid') {
        newErrors.cnicFront = validationStatus.cnicFront.message || 'CNIC Front must pass validation';
      }

      if (!documents.cnicBack) {
        newErrors.cnicBack = 'CNIC Back Side is required';
      } else if (validationStatus.cnicBack.status !== 'valid') {
        newErrors.cnicBack = validationStatus.cnicBack.message || 'CNIC Back must pass validation';
      }
      
      if (Object.keys(newErrors).length > 0) {
        setFieldErrors(newErrors);
        setError('Please upload valid documents');
        return;
      }
    }
                ...
                {step === 2 && (
                  <View className="gap-4">
                    ...
                    <TouchableOpacity
                      ...
                      disabled={validationStatus.license.status === 'validating'}
                      className={`p-6 border border-dashed rounded-xl bg-white items-center flex-row gap-4 ${
                        validationStatus.license.status === 'valid' ? 'border-green-500 bg-green-50/10' :
                        validationStatus.license.status === 'invalid' ? 'border-red-500 bg-red-50/10' :
                        fieldErrors.license ? 'border-red-500' : 'border-gray-300'
                      }`}
                    >
                      <View className="w-12 h-12 bg-purple-100 rounded-full items-center justify-center">
                        {validationStatus.license.status === 'valid' ? (
                    ...
                    {validationStatus.license.status === 'validating' && (
                      <View className="flex-row items-center gap-2 mt-1 ml-1">
                        <ActivityIndicator size="small" color="#9333ea" />
                        <Text className="text-purple-600 text-xs font-bold">🔍 Validating document...</Text>
                      </View>
                    )}
                    {validationStatus.license.status === 'valid' && (
                      <Text className="text-green-600 text-xs mt-1 ml-1 font-bold">✅ Valid Medical Document</Text>
                    )}
                    {validationStatus.license.status === 'invalid' && (
                      <Text className="text-red-600 text-xs mt-1 ml-1 font-bold">❌ Invalid Upload: {validationStatus.license.message}</Text>
                    )}
                    {fieldErrors.license && validationStatus.license.status !== 'invalid' && (
                      <Text className="text-red-500 text-xs mt-1 ml-1">{fieldErrors.license}</Text>
                    )}

                    {/* CNIC Front */}
                    <TouchableOpacity
                      onPress={() => handleFileUpload('cnicFront')}
                      disabled={validationStatus.cnicFront.status === 'validating'}
                      className={`p-6 border border-dashed rounded-xl bg-white items-center flex-row gap-4 ${
                        validationStatus.cnicFront.status === 'valid' ? 'border-green-500 bg-green-50/10' :
                        validationStatus.cnicFront.status === 'invalid' ? 'border-red-500 bg-red-50/10' :
                        fieldErrors.cnicFront ? 'border-red-500' : 'border-gray-300'
                      }`}
                    >
                      <View className="w-12 h-12 bg-purple-100 rounded-full items-center justify-center">
                        {validationStatus.cnicFront.status === 'valid' ? (
                          <CheckCircle size={24} color="#16a34a" />
                        ) : (
                          <Upload size={24} color="#9333ea" />
                        )}
                    ...
                    {validationStatus.cnicFront.status === 'validating' && (
                      <View className="flex-row items-center gap-2 mt-1 ml-1">
                        <ActivityIndicator size="small" color="#9333ea" />
                        <Text className="text-purple-600 text-xs font-bold">🔍 Validating document...</Text>
                      </View>
                    ...
                    {validationStatus.cnicFront.status === 'valid' && (
                      <Text className="text-green-600 text-xs mt-1 ml-1 font-bold">✅ Valid CNIC Front</Text>
                    )}
                    {validationStatus.cnicFront.status === 'invalid' && (
                      <Text className="text-red-600 text-xs mt-1 ml-1 font-bold">❌ Invalid Upload: {validationStatus.cnicFront.message}</Text>
                    )}
                    {fieldErrors.cnicFront && validationStatus.cnicFront.status !== 'invalid' && (
                      <Text className="text-red-500 text-xs mt-1 ml-1">{fieldErrors.cnicFront}</Text>
                    )}

                    {/* CNIC Back */}
                    <TouchableOpacity
                      onPress={() => handleFileUpload('cnicBack')}
                      disabled={validationStatus.cnicBack.status === 'validating'}
                      className={`p-6 border border-dashed rounded-xl bg-white items-center flex-row gap-4 ${
                        validationStatus.cnicBack.status === 'valid' ? 'border-green-500 bg-green-50/10' :
                        validationStatus.cnicBack.status === 'invalid' ? 'border-red-500 bg-red-50/10' :
                        fieldErrors.cnicBack ? 'border-red-500' : 'border-gray-300'
                      }`}
                    >
                      <View className="w-12 h-12 bg-purple-100 rounded-full items-center justify-center">
                        {validationStatus.cnicBack.status === 'valid' ? (
                          <CheckCircle size={24} color="#16a34a" />
                        ) : (
                          <Upload size={24} color="#9333ea" />
                        )}
                    ...
                    {validationStatus.cnicBack.status === 'validating' && (
                      <View className="flex-row items-center gap-2 mt-1 ml-1">
                        <ActivityIndicator size="small" color="#9333ea" />
                        <Text className="text-purple-600 text-xs font-bold">🔍 Validating document...</Text>
                      </View>
                    ...
                    {validationStatus.cnicBack.status === 'valid' && (
                      <Text className="text-green-600 text-xs mt-1 ml-1 font-bold">✅ Valid CNIC Back</Text>
                    )}
                    {validationStatus.cnicBack.status === 'invalid' && (
                      <Text className="text-red-600 text-xs mt-1 ml-1 font-bold">❌ Invalid Upload: {validationStatus.cnicBack.message}</Text>
                    )}
                    {fieldErrors.cnicBack && validationStatus.cnicBack.status !== 'invalid' && (
                      <Text className="text-red-500 text-xs mt-1 ml-1">{fieldErrors.cnicBack}</Text>
                    )}

                    {/* Educational Certificate */}
                    <TouchableOpacity
                      onPress={() => handleFileUpload('certificate')}
                      disabled={validationStatus.certificate.status === 'validating'}
                      className={`p-6 border border-dashed rounded-xl bg-white items-center flex-row gap-4 ${
                        validationStatus.certificate.status === 'valid' ? 'border-green-500 bg-green-50/10' :
                        validationStatus.certificate.status === 'invalid' ? 'border-red-500 bg-red-50/10' :
                        'border-gray-300'
                      }`}
                    >
                      <View className="w-12 h-12 bg-purple-100 rounded-full items-center justify-center">
                        {validationStatus.certificate.status === 'valid' ? (
                          <CheckCircle size={24} color="#16a34a" />
                        ) : (
                          <Upload size={24} color="#9333ea" />
                        )}
                    ...
                    {validationStatus.certificate.status === 'validating' && (
                      <View className="flex-row items-center gap-2 mt-1 ml-1">
                        <ActivityIndicator size="small" color="#9333ea" />
                        <Text className="text-purple-600 text-xs font-bold">🔍 Validating document...</Text>
                      </View>
                    ...
                    {validationStatus.certificate.status === 'valid' && (
                      <Text className="text-green-600 text-xs mt-1 ml-1 font-bold">✅ Valid Document</Text>
                    )}
                    {validationStatus.certificate.status === 'invalid' && (
                      <Text className="text-red-600 text-xs mt-1 ml-1 font-bold">❌ Invalid Upload: {validationStatus.certificate.message}</Text>
                    )}
